...
export default function AdditionalInfo() {
    ...
    useEffect(() => {
        if (userInfo.signupType !== 'kakao') {
            updateUserInfo({
                ...
                nickname: userInfo.kakaoNickname || '카카오닉네임',
                email: userInfo.kakaoEmail || 'kakaouser@kakao.com',
                phone: '', address: '', signupType: 'kakao',
    ...
    const handleDuplicateCheck = async () => {
        ...
        if (value === originalKakaoNickname) {
            setValidationStates(prev => ({ ...prev, nickname: { status: 'success', message: '✅ 카카오 닉네임이 설정되었습니다', checked: true } }));
            return;
        }
        ...
        setValidationStates(prev => ({ ...prev, nickname: { status: 'loading', message: '🔄 확인 중...', checked: false } }));

        try {
            const result = await checkDuplicate('nickname', value);
            setValidationStates(prev => ({ ...prev, nickname: { status: result.available ? 'success' : 'error', message: result.available ? '✅ 사용 가능한 닉네임입니다' : `❌ ${result.message}`, checked: result.available } }));
        } catch (error) {
            setValidationStates(prev => ({ ...prev, nickname: { status: 'error', message: '❌ 확인 중 오류가 발생했습니다', checked: false } }));
        }
    };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (isFormValid) {
            const finalUserData = { ...userInfo, ...formData, agreements: { ...userInfo.agreements, privacy: formData.agreeToTerms } };
                ...
                <div className="card-content">
                    ...
                    <form className="form-container" onSubmit={handleSubmit}>
                        <div className="input-group">
                            <input className="input-field" type="text" value={formData.name} onChange={(e) => handleInputChange('name', e.target.value)} placeholder="이름을 입력하세요" required />
                        </div>
                        <div className="input-group">
                            <input className={`input-field ${validationStates.nickname.status}`} type="text" value={formData.nickname} onChange={(e) => handleInputChange('nickname', e.target.value)} placeholder="닉네임을 입력하세요 (필수, 2~10자)" required />
                            <button className="duplicate-check-btn" type="button" onClick={handleDuplicateCheck} disabled={validationStates.nickname.status === 'loading'}>
                        ...
                        <div className="input-group">
                            <input className="input-field" type="text" value={formData.address} placeholder="주소를 검색하세요" onClick={handleAddressSearch} readOnly required />
                            {!formData.address && (
                        ...
                        <div className="agreement-container">
                            <label className="checkbox-label">
                                <input className="checkbox-input" type="checkbox" checked={formData.agreeToTerms} onChange={(e) => handleAgreementChange(e.target.checked)} />
                        ...
                        <button className={`submit-button ${isFormValid ? 'active' : ''}`} type="submit" disabled={!isFormValid} style={{ backgroundColor: isFormValid ? '#85B3EB' : '#CBD5E1', cursor: isFormValid ? 'pointer' : 'not-allowed', transition: 'all 0.3s ease' }}>
                            가입 완료
                        </button>
                    </form>
                </div>
                ...
                <div className="postcode-overlay">
                    <div className="postcode-modal">
                        <div className="postcode-header">
                            <h3>주소 검색</h3>
                            <button className="postcode-close" onClick={() => setIsPostcodeOpen(false)}>×</button>
                        </div>
                        <DaumPostcode onComplete={handleAddressComplete} autoClose={false} style={{ width: '100%', height: '400px' }} />
                    </div>
                </div>
            )}
        </div>
